...
ROOT=Path(__file__).resolve().parents[1]; MANIFEST=ROOT/"results"/"manifest_v3.7.json"
...
def main():
    failures=[]
    if not MANIFEST.exists(): raise SystemExit("missing results/manifest_v3.7.json")
    m=json.loads(MANIFEST.read_text(encoding="utf-8"))
    if m.get("release")!="v3.7" or m.get("package_version")!="3.7.0": failures.append("manifest release/version mismatch")
    ...
    try:
        ...
        if dist!="3.7.0": failures.append(f"distribution version is {dist}")
        if commlab.__version__!="3.7.0": failures.append(f"runtime version is {commlab.__version__}")
    ...
    print(f"v3.7 release verification OK: {checked} hashed result artifacts; summary={summary}")
