...
SCRIPTS=["v36_exploration_overlap.py","v36_clipping_frontier.py","v36_model_misspecification.py",
         "v36_support_boundary.py","v36_temporal_reuse.py","v36_policy_selection.py"]
...
def main():
    ...
    for i,name in enumerate(SCRIPTS,1):
        ...
        env=base.copy(); env["PYTHONPATH"]=str(ROOT/"src"); env["MPLCONFIGDIR"]=str(ROOT/".mplcache"/"v36_suite"/Path(name).stem)
    ...
    print(f"v3.6 suite complete: {len(SCRIPTS)} experiments in {time.perf_counter()-start:.1f}s",flush=True)
