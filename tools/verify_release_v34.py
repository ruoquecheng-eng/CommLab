...
MANIFEST=ROOT/"results"/"manifest_v3.4.json"
REQ_CSV={
    "v34_drift_adaptation.csv","v34_localized_risk.csv","v34_feedback_delay.csv",
    "v34_adaptation_gain.csv","v34_target_frontier.csv","v34_correlation_reallocation.csv",
}
REQ_PNG={
    "v34_drift_adaptation_reliability.png","v34_drift_adaptation_spend.png",
    "v34_localized_critical_risk.png","v34_localized_budget_crossover.png",
    "v34_feedback_delay_reliability.png","v34_feedback_delay_activity.png",
    "v34_adaptation_gain_risk.png","v34_adaptation_gain_activity.png",
    "v34_target_resource_frontier.png","v34_target_infeasibility.png",
    "v34_correlation_action_mix.png","v34_correlation_reliability.png",
}
...
def main():
    failures=[]
    if not MANIFEST.exists(): raise SystemExit("missing results/manifest_v3.4.json")
    m=json.loads(MANIFEST.read_text(encoding="utf-8"))
    if m.get("release")!="v3.4" or m.get("package_version")!="3.4.0": failures.append("manifest release/version mismatch")
    ...
    failures += [f"missing v3.4 dataset: {x}" for x in sorted(REQ_CSV-actual_csv)]
    failures += [f"missing v3.4 figure: {x}" for x in sorted(REQ_PNG-actual_png)]
    ...
    required=[
        "README.md","PROJECT_STATUS.md","RELEASE_NOTES_v3.4.md","docs/technical_report.md",
        "docs/portfolio_summary_v3.4.md","docs/reproducibility_v3.4.md","docs/experiment_catalog_v3.4.md",
        "docs/v3.4_tradeoff_matrix.md","docs/methodological_inspirations_v3.4.md",
        "docs/v3.4_adaptive_risk_architecture.md","docs/release_acceptance_v3.4.md",
        "tools/build_manifest_v34.py","tools/verify_release_v34.py","tools/run_v34_suite.py",
        "tools/run_release_acceptance_v34.py",
    ]
    failures += [f"missing release file: {x}" for x in required if not (ROOT/x).exists()]
    try:
        import commlab
        dist=importlib.metadata.version("commlab-ofdm")
        if dist!="3.4.0": failures.append(f"distribution version is {dist}")
        if commlab.__version__!="3.4.0": failures.append(f"runtime version is {commlab.__version__}")
    ...
    print(f"v3.4 release verification OK: {checked} hashed result artifacts; summary={summary}")
