...
def main():
    steps=[
        ...
        ("v3.7.1 experiment suite",[sys.executable,"tools/run_v37_suite.py"],False),
        ("desktop source smoke test",[sys.executable,"desktop/launcher.py","--smoke-test"],False),
        ...
        ("version/import path",[sys.executable,"-c",
            ...
            "assert importlib.metadata.version('commlab-ofdm')=='3.7.1'; "
            "assert commlab.__version__=='3.7.1'; "
        ...
        ("pytest without project PYTHONPATH",[sys.executable,"-m","pytest","-q"],True),
        ("compileall",[sys.executable,"-m","compileall","-q","src","app","desktop","experiments","tools"],True),
        ("build manifest",[sys.executable,"tools/build_manifest_v371.py"],True),
        ("verify manifest/release",[sys.executable,"tools/verify_release_v371.py"],True),
    ]
    timings=[]; start=time.perf_counter()
    for label,cmd,clear in steps: timings.append((label,run(label,cmd,clear)))
    total=time.perf_counter()-start
    print("\n=== CommLab v3.7.1 acceptance PASSED ===",flush=True)
