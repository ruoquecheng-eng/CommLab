...
ROOT=Path(__file__).resolve().parents[1]; MANIFEST=ROOT/"results"/"manifest_v3.6.json"
REQ_CSV={"v36_exploration_overlap.csv","v36_clipping_frontier.csv","v36_model_misspecification.csv",
         "v36_support_boundary.csv","v36_temporal_reuse.csv","v36_policy_selection.csv"}
REQ_PNG={"v36_exploration_overlap_error.png","v36_exploration_overlap_support.png",
         "v36_clipping_frontier_error.png","v36_clipping_frontier_bias_variance.png",
         "v36_model_misspecification_error.png","v36_model_misspecification_bias.png",
         "v36_support_boundary_error.png","v36_support_boundary_mass.png",
         "v36_temporal_reuse_current_error.png","v36_temporal_reuse_value.png",
         "v36_policy_selection_regret.png","v36_policy_selection_fallback.png"}
...
def main():
    failures=[]
    if not MANIFEST.exists(): raise SystemExit("missing results/manifest_v3.6.json")
    m=json.loads(MANIFEST.read_text(encoding="utf-8"))
    if m.get("release")!="v3.6" or m.get("package_version")!="3.6.0": failures.append("manifest release/version mismatch")
    ...
    failures += [f"missing v3.6 dataset: {x}" for x in sorted(REQ_CSV-actual_csv)]
    failures += [f"missing v3.6 figure: {x}" for x in sorted(REQ_PNG-actual_png)]
    ...
    required=["README.md","PROJECT_STATUS.md","RELEASE_NOTES_v3.6.md","docs/technical_report.md",
        "docs/portfolio_summary_v3.6.md","docs/reproducibility_v3.6.md","docs/experiment_catalog_v3.6.md",
        "docs/v3.6_tradeoff_matrix.md","docs/methodological_inspirations_v3.6.md",
        "docs/v3.6_offline_evaluation_architecture.md","docs/release_acceptance_v3.6.md",
        "tools/build_manifest_v36.py","tools/verify_release_v36.py","tools/run_v36_suite.py",
        "tools/run_release_acceptance_v36.py"]
    failures += [f"missing release file: {x}" for x in required if not (ROOT/x).exists()]
    try:
        import commlab
        dist=importlib.metadata.version("commlab-ofdm")
        if dist!="3.6.0": failures.append(f"distribution version is {dist}")
        if commlab.__version__!="3.6.0": failures.append(f"runtime version is {commlab.__version__}")
    ...
    print(f"v3.6 release verification OK: {checked} hashed result artifacts; summary={summary}")
