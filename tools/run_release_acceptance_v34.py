...
def main():
    steps=[
        ...
        ("v3.4 experiment suite",[sys.executable,"tools/run_v34_suite.py"],False),
        ...
        ("version/import path",[sys.executable,"-c",
            ...
            "assert importlib.metadata.version('commlab-ofdm')=='3.4.0'; "
            "assert commlab.__version__=='3.4.0'; "
        ...
        ("build manifest",[sys.executable,"tools/build_manifest_v34.py"],True),
        ("verify manifest/release",[sys.executable,"tools/verify_release_v34.py"],True),
    ]
    timings=[]; start=time.perf_counter()
    for label,cmd,clear in steps: timings.append((label,run(label,cmd,clear)))
    total=time.perf_counter()-start
    print("\n=== CommLab v3.4 acceptance PASSED ===",flush=True)
